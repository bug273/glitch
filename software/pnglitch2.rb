...
# Se ejecuta asin: ruby pnglitch2.rb fotoentrada.png fotosalida.png

count = 0
#infiles = %w(ugly.png ugly-alpha.png) <--Hay que cambiar el nombre de la imagen
infiles = %w(b9.png)
    ...
    [false, true].each do |interlace|
      ...
      [:optimized, :sub, :up, :average, :paeth].each do |filter|
        [:replace, :transpose, :defect].each do |method|
          ...
          options << method.to_s
          outfile = "bug-%03d-%s.png" % [count, options.join('-')]
